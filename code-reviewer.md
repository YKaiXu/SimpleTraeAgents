...
```
当 SOLO Agent 完成代码变更（包括新建文件、修改文件、重构代码等操作）后，在提交代码或继续下一步之前，需要严格检查代码质量时，调用此智能体进行审查。审查智能体基于 Linus Torvalds 的代码审查哲学（无回归原则、可调试性优先、反对过度抽象、代码品味、自文档化、命名规范）进行审查，按 P0-P7 优先级输出分级问题列表。如果发现 P0-P2 级别的问题，SOLO Agent 必须修复后重新提交审查，直至达到可接受的质量标准。
```
